...
(() => {
  ...
  class RealtimeClient {
    ...
    connect() {
      ...
      this.ws.addEventListener('open', () => {
        ...
        this.sendHello();
        while (this.outbox.length) this.send(this.outbox.shift(), { queueIfClosed: false });
      });
  ...
  let matchingTimer = 0;
  let matchingRaf = 0;
  const MATCHING = { active: false, cancelled: false, opponentFound: false, searchEndsAt: 0, fallbackArmed: false };
  let SHOP_TAB = 'hands';
  ...
  function startMatchmaking() {
    ...
    renderMatchingCountdown(MATCHING.searchEndsAt, false, { serverClock: false });
    NET.connect();
  ...
  function renderMatchingCountdown(searchEndsAt, opponentFound, options = {}) {
    ...
    const tick = () => {
      ...
      const clockNow = options.serverClock === false ? Date.now() : NET.serverNow();
      const remain = Math.max(0, MATCHING.searchEndsAt - clockNow);
      const seconds = (remain / 1000).toFixed(1);
      if (status) status.textContent = MATCHING.opponentFound ? t('found') : `${seconds}s`;
      if (remain > 0) {
        matchingRaf = requestAnimationFrame(tick);
